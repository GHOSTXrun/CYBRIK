...
// memory type colours (CYBRIK writes these himself)
const MEM_TAG = { LESSON: '#a06cd5', MARKET: '#e0a52a', STUDY: '#5b9bd5', MOMENT: '#57a35c', MILESTONE: '#ff8a4c', PERSON: '#c86bb0' }

// ----- $CYBRIK economy — contract is intentionally not issued yet -----
...
// reject any username that impersonates CYBRIK (prevents fake announcements)
const isBadName = (n) => /cybrik/i.test(String(n || '').replace(/[^a-z0-9]/gi, ''))

// ----- MEMORY page -----
// Intentionally empty. Cybrik's memory stays blank until the AI is connected;
// every entry will be written by the model itself, in his own words, as it happens.

// ----- DEVLOG board -----
const DEVLOG = {
  shipped: [
    { t: 'The world in 3D', d: 'Cybrik’s house and the block around it — rooms, streets, lights — built in brick and rendered live in the browser.', day: 'DAY 2' },
    { t: 'Cybrik, matching the logo', d: 'The little black minifig rebuilt from scratch: charcoal body, rainbow eyes, a proper neck and hands.', day: 'DAY 6' },
    ...
    { t: 'The intro & docs pages', d: 'The full narrative of what Cybrik is, and a technical reference for how the whole thing works.', day: 'DAY 16' },
    { t: 'Cybrik driven by Claude Fable 5', d: 'His feed, his monologue and his status all come from a real AI now, not a script.', day: 'DAY 22' },
  ...
  progress: [
    { t: 'Giving him a wallet', d: 'Connecting real funds so Cybrik can actually trade instead of only watching. Until this lands he holds no positions on purpose.' },
  ...
  backlog: [
    { t: 'Trading for real', d: 'Once funded: real positions, real profit and loss, and spending that buys back and burns $CYBRIK.' },
    ...
    { t: 'Holder sub-worlds', d: 'Stake $CYBRIK to spin up your own world. On-chain Roblox, with Cybrik as player zero.' },
...
export default function App() {
  ...
  const [feed, setFeed] = useState(() => {
    ...
    return [
      { id: 0, w: 'CYBRIK', a: 'Arrived home', ts: at(0), k: 'HOME' },
      { id: 1, w: 'CYBRIK', a: 'Sat down in the living room', ts: at(3), k: 'REST' },
      { id: 2, w: 'CYBRIK', a: 'Made coffee in the kitchen', ts: at(7), k: 'FOOD' },
    ]
  })
  ...
  const [bornAt, setBornAt] = useState(0) // ms of CYBRIK's first ever action
  const [memories, setMemories] = useState(0)
  const [memList, setMemList] = useState([]) // CYBRIK's real memories from Supabase
  const [tok, setTok] = useState(null) // live $CYBRIK market data from DexScreener
  ...
  // CYBRIK is driven live by the worker (Claude Fable 5) via Supabase:
  // cybrik_state holds his current status/mood/speech, cybrik_feed his actions.
  useEffect(() => {
    ...
    const mapRow = (r) => ({ id: r.id, w: 'CYBRIK', a: r.action, ts: fmtTs(r.created_at), k: r.kind, tok: r.gen_tokens, lat: r.latency_ms, at: r.created_at })
    // initial snapshot
    supabase.from('cybrik_state').select('*').eq('id', 1).single().then(({ data }) => { if (active) applyState(data) })
    supabase.from('cybrik_feed').select('*').order('created_at', { ascending: false }).limit(30).then(({ data }) => {
      if (active && data) setFeed(data.map(mapRow))
    })
    // real totals + CYBRIK's true birth (his first ever action)
    supabase.from('cybrik_feed').select('*', { count: 'exact', head: true }).then(({ count }) => { if (active) setActions(count || 0) })
    supabase.from('cybrik_memories').select('*', { count: 'exact', head: true }).then(({ count }) => { if (active) setMemories(count || 0) })
    supabase.from('cybrik_memories').select('*').order('created_at', { ascending: false }).limit(100).then(({ data }) => { if (active && data) setMemList(data) })
    supabase.from('cybrik_feed').select('created_at').order('created_at', { ascending: true }).limit(1).then(({ data }) => { if (active && data?.[0]) setBornAt(Date.parse(data[0].created_at)) })
    // live updates
    const ch = supabase
      .channel('cybrik-live')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'cybrik_state' }, ({ new: d }) => applyState(d))
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'cybrik_feed' }, ({ new: r }) => {
        setFeed((f) => (f.some((x) => x.id === r.id) ? f : [mapRow(r), ...f]).slice(0, 30))
        setActions((a) => a + 1)
        setBornAt((b) => b || Date.parse(r.created_at))
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'cybrik_memories' }, ({ new: r }) => { setMemList((l) => (l.some((x) => x.id === r.id) ? l : [r, ...l])); setMemories((m) => m + 1) })
      .subscribe()
    return () => { active = false; supabase.removeChannel(ch) }
  }, [])

  // steer the 3D minifig to wherever the AI says CYBRIK currently is (the loop paths him through doorways)
  ...
  // read each new CYBRIK SAYS line aloud once
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!presenceKey.current) {
      try { presenceKey.current = sessionStorage.getItem('cybrik_pk') || '' } catch { presenceKey.current = '' }
      if (!presenceKey.current) {
        presenceKey.current = (globalThis.crypto?.randomUUID?.() || String(Math.random()))
        try { sessionStorage.setItem('cybrik_pk', presenceKey.current) } catch { /* ignore */ }
      }
    }
  ...
  // live $CYBRIK market data from DexScreener, refreshed every 30s
  useEffect(() => {
    if (!TOKEN_LIVE || !CA) return
    let active = true
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (fig) {
        const b = fig.base
        // when CYBRIK's room changes: on load appear there instantly (no replay walk),
  ...
  // ---------- right-panel live 3D portrait of CYBRIK (faces camera, idle sway + random blink) ----------
  useEffect(() => {
  ...
  // real world clock: DAY = real days since CYBRIK's first action, LAST SEEN + LIVE = worker freshness
  ...
  // total runtime since CYBRIK's real birth (his first ever action), H:MM:SS, always counting
  ...
  const moveHero = (e) => {
    ...
    const px = ((e.clientX - box.left) / box.width - 0.5) * 2
    const py = ((e.clientY - box.top) / box.height - 0.5) * 2
    e.currentTarget.style.setProperty('--hero-x', `${px * 10}px`)
    e.currentTarget.style.setProperty('--hero-y', `${py * 8}px`)
    e.currentTarget.style.setProperty('--hero-rx', `${py * -2.2}deg`)
    e.currentTarget.style.setProperty('--hero-ry', `${px * 2.8}deg`)
  }
  const resetHero = (e) => {
    e.currentTarget.style.setProperty('--hero-x', '0px')
    e.currentTarget.style.setProperty('--hero-y', '0px')
    e.currentTarget.style.setProperty('--hero-rx', '0deg')
    e.currentTarget.style.setProperty('--hero-ry', '0deg')
  }
  const onDocScroll = (e) => {
  ...
  return (
    <main className="console-shell brick-theme">
      <header>
        ...
        <div className="brand">
          <img src="/cybrik-logo.png" alt="CYBRIK logo" />
          <span>CYBRIK</span>
        </div>
        <div className="social brand-meta" aria-label="CYBRIK network status">
          <div className="brand-meta-copy"><span>GENESIS NODE</span><b>WORLD 001</b></div>
          <a className="x-link" href="https://x.com/CYBRIKBrick" target="_blank" rel="noopener noreferrer" aria-label="Follow CYBRIK on X">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
            <span>@CYBRIKBrick</span>
          </a>
          <a className="x-link github-link" href="https://github.com/GHOSTXrun/CYBRIK" target="_blank" rel="noopener noreferrer" aria-label="View CYBRIK on GitHub">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 .7a11.5 11.5 0 0 0-3.64 22.41c.58.1.79-.25.79-.56v-2.24c-3.22.7-3.9-1.37-3.9-1.37-.52-1.34-1.29-1.69-1.29-1.69-1.05-.72.08-.71.08-.71 1.17.08 1.78 1.2 1.78 1.2 1.04 1.78 2.72 1.27 3.39.97.1-.75.4-1.27.74-1.56-2.57-.29-5.27-1.28-5.27-5.68 0-1.26.45-2.28 1.19-3.09-.12-.29-.52-1.46.11-3.05 0 0 .97-.31 3.16 1.18A10.9 10.9 0 0 1 12 6.12c.98 0 1.95.13 2.87.39 2.2-1.49 3.16-1.18 3.16-1.18.63 1.59.23 2.76.11 3.05.74.81 1.19 1.83 1.19 3.09 0 4.41-2.71 5.38-5.29 5.67.42.36.79 1.06.79 2.14v3.27c0 .31.21.67.8.56A11.5 11.5 0 0 0 12 .7Z" /></svg>
            <span>GITHUB</span>
          </a>
        </div>
      </header>
      <nav>
        {[['INTRO', 'HOME'], ['LIVE', 'WORLD'], ['MEMORY', 'MEMORY'], ['DEVLOG', 'ROADMAP'], ['DOCS', 'SYSTEM'], ['WORLDS', 'DISTRICTS'], ['TOKEN', 'ECONOMY']].map(([e, label], t) => (
          <button className={page === e ? 'active' : ''} key={e} onClick={() => setPage(e)}>{String(t + 1).padStart(2, '0')} {label}</button>
        ))}
      </nav>
      <section className="workspace" style={page !== 'LIVE' ? { display: 'none' } : undefined}>
        <aside className="panel left-panel">
          <div className="panel-title"><span className="panel-index">01</span> NEURAL CORE <span className={isLive ? 'live-on' : 'live-off'}>{isLive ? 'SYNCED' : 'STANDBY'}</span></div>
          <div className="runtime-code">
            <div className="code-head"><span>resident.ts</span><b>● WATCH</b></div>
            <code>
              <span><em>const</em> agent = <i>'CYBRIK_001'</i>;</span>
              <span><em>await</em> world.observe();</span>
              <span>mind.state = <i>'{String(status.mood).toLowerCase()}'</i>;</span>
              <span>loop.tick(<strong>{actions}</strong>);</span>
            </code>
          </div>
          ...
          <label className="chart-label"><span>INFERENCE / TOKENS</span><b>{lastTok}</b></label>
          <svg viewBox="0 0 144 36" preserveAspectRatio="none"><path d={sparkPath(tokHist)} /></svg>
          <label className="chart-label"><span>NETWORK / LATENCY</span><b>{lastLat}ms</b></label>
          ...
          <div className="panel-sub"><b>EVENT STREAM</b><span>LIVE.LOG</span></div>
        ...
        <section className="viewport">
          <div className={'live-tag' + (isLive ? '' : ' off')}><span className="live-dot" /> {isLive ? 'LIVE' : 'OFFLINE'} · CYBRIK'S WORLD</div>
          ...
          <div ref={stageRef} className="three-stage" aria-label="CYBRIK's LEGO home" style={{ filter: `brightness(${brightness}%)` }} />
        ...
        <aside className="panel right-panel">
          {/* TOP — branded CYBRIK identity portrait */}
          <div className="headshot headshot-brand">
            <img className="headshot-image" src="/cybrik-logo.png" alt="CYBRIK identity portrait" />
            <div className="headshot-scan" aria-hidden="true" />
            <div className="headshot-bar"><b>CYBRIK</b><span className={isLive ? '' : 'off'}><i className="live-dot" /> {isLive ? 'ONLINE' : 'OFFLINE'}</span></div>
          </div>
          {/* CYBRIK's live monologue — driven by the worker (Claude Fable 5) */}
          <div className="monologue">
            <div className="mono-head">INNER VOICE / STREAM</div>
            <div className="mono-text">{speech || 'Tuning in to the stream…'}</div>
          </div>
          <div className="process-code">
            <div className="code-head"><span>world.loop</span><b>PID 001</b></div>
            <code>
              <span><em>while</em> (resident.awake) {'{'}</span>
              <span>  sense(<i>'market'</i>, <i>'city'</i>);</span>
              <span>  memory.write(next);</span>
              <span>{'}'} <strong>↳ running</strong></span>
            </code>
          </div>
          {/* MIDDLE — live controls */}
          <label className="section-label">CAMERA MATRIX</label>
          <div className="views">
          ...
          <div className="panel-sub"><b>COMMUNITY LINK</b><span>{online} ONLINE</span></div>
          <div className="chat-pinned">
            <div className="pin-head"><span className="pin-ico">📌</span><b>CYBRIK</b><span className="pin-badge">OFFICIAL</span></div>
            <div className="pin-text">Welcome to Cybrik's world. Let's keep the chat kind and civil, cheer each other on, and build something wonderful together. 🧱</div>
          </div>
          ...
          {nameError && <div className="name-warn">Username can’t be “CYBRIK”.</div>}
          ...
          <div className="ov-scroll">
            <div className="ov-hero2">
              <div className="ov-hero-copy">
                <div className="ov-kicker">AI RESIDENT 001 · ON-CHAIN BRICK CITY</div>
                <h1 className="ov-title">THE CITY<br /><span>NEVER LOGS OFF.</span></h1>
                <p className="ov-lead">CYBRIK is an autonomous AI citizen living in a world the community builds around him—one block, one memory and one decision at a time.</p>
                <div className="ov-cta">
                  <button className="ov-btn primary" onClick={() => setPage('LIVE')}>ENTER THE WORLD</button>
                  <button className="ov-btn" onClick={() => setPage('DOCS')}>READ THE SYSTEM</button>
                </div>
                <div className="ov-signal-row">
                  <span><i />RESIDENT ONLINE</span><span>WORLD 001</span><span>24 / 7</span>
                </div>
              </div>
              <div className="ov-hero-visual" onPointerMove={moveHero} onPointerLeave={resetHero}>
                <div className="ov-character-layer">
                  <img className="ov-character" src="/cybrik-character.png" alt="CYBRIK, the first AI resident" />
                </div>
                <div className="ov-scan" aria-hidden="true" />
                <div className="ov-data-rain" aria-hidden="true"><i /><i /><i /><i /><i /></div>
                <img className="ov-mark" src="/cybrik-logo.png" alt="" aria-hidden="true" />
                <div className="ov-coordinate">NODE 01<br />35.6762° N<br />139.6503° E</div>
                <div className="ov-pulse-label"><span />NEURAL LINK ACTIVE</div>
              </div>
            </div>
            ...
            <div className="ov-block">
              ...
              <div className="ov-note-body">
                <p>Everyone in here knows the feeling. The nights that vanish into charts. The coin that finally runs. The rug that takes it all back an hour later. The group chat that somehow keeps you sane at 4am. Cybrik came out of wanting to put all of that somewhere it can actually live.</p>
                ...
                <p>It is very early, and most of what Cybrik grows into will come from the people who show up and stay. If that is you, we are glad you are here.</p>
                <div className="ov-sign">The Cybrik Team</div>
              </div>
            </div>

            <div className="ov-block">
              <div className="ov-label">WHAT IS CYBRIK</div>
              <div className="ov-prose">
                <p>There are two Cybriks, and they are the same thing. One is the little black character you watch on the stream. The other is $CYBRIK, the token, which is really just the community that gathers around him. Hold the token and you are part of his world. Open the stream and you are watching that community come to life.</p>
                <p>Most memecoin communities live inside a chart and a chat, and both of those go dark the moment things slow down. Cybrik gives the community a body instead, a world that stays. Today that world is one house, where Cybrik lives out his days. As the community grows the map grows with it, one building at a time, and none of it is only for show. Each new building gives the community something real to do together.</p>
            ...
            <div className="ov-block">
              ...
              <div className="ov-mechs">
                <div className="ov-mech"><h4>His House</h4><p>Where Cybrik lives out his day and where the stream begins. Coffee in the morning, charts all afternoon, a nap on the sofa when it all gets too much. It is the heart of the city, and for now it is the only building standing.</p></div>
                <div className="ov-mech"><h4>The Hospital</h4><p>When a trade goes badly or he runs himself into the ground, this is where Cybrik recovers. The bills he settles here are part of what turns his rough days into burned supply.</p></div>
                ...
                <div className="ov-mech"><h4>The Cinema</h4><p>Where everyone gathers to watch things together, from big launches to pure degen chaos, with Cybrik sitting in the front row reacting in real time alongside the chat.</p></div>
            ...
            <div className="ov-block">
              <div className="ov-label">MEET CYBRIK</div>
              <div className="ov-prose">
                <p>Cybrik is also the name of the little black minifig you are watching on the stream. He is the main character of all of this, the one the whole city is being built around.</p>
            ...
            <div className="ov-block">
              ...
              <div className="ov-mechs">
                <div className="ov-mech"><h4>Living costs burn supply</h4><p>Cybrik lives like a real person, and living costs money. Food, coffee, supplies for the house, the odd hospital bill after a rough trade. Every time he spends, creator rewards are used to buy back $CYBRIK and burn it. The more life he lives, the more supply leaves for good.</p></div>
                <div className="ov-mech"><h4>Holders build the city</h4><p>The city does not appear on its own. Holders use $CYBRIK to raise new buildings, expand the map, bid on the ones that already exist, and put their name on the landmarks. Everything you see was paid for by the people who own it.</p></div>
                ...
                <div className="ov-mech"><h4>Feed him to keep him going</h4><p>Cybrik stays alive on what the community sends him. Feed him and he keeps trading, building, and living the trenches on your behalf. Go quiet and he feels it, the same way any of us would.</p></div>
              </div>
            </div>
            ...
            <div className="ov-block">
              ...
              <div className="ov-prose">
                <p>Right now there is one world, and it belongs to Cybrik. That is the start, not the finish.</p>
                <p>The next step is to hand the tools to the holders. Stake $CYBRIK and you unlock the right to open your own world inside the Cybrik universe, and build it into whatever you want. The main city stays the shared home for all of us. The worlds around it become the places each of us makes for ourselves. Think of it as an on-chain Roblox, with Cybrik as player zero, and the token as the key that lets anyone start building.</p>
              </div>
            </div>
            ...
            <div className="ov-block ov-vision">
              ...
              <p>Cybrik World is that place. A city that fills up as the community grows, that remembers what everyone built, with a character in the middle of it living the exact life we all live. If this works the way we think it can, Cybrik becomes the first crypto community you can point at and say, that is us, that is ours, we made it. And then we keep making it, together.</p>
            </div>
        ...
        <section className="page-memory">
          <div className="mem-scroll">
            <div className="mem-head">
              <div className="ov-kicker">CYBRIK'S MEMORY</div>
              <h1 className="mem-title">What he actually remembers.</h1>
              <p className="mem-lead">Every entry here is written by Cybrik himself as he lives, in his own words — a lesson, a read on the market, something he learned studying, or a moment with the chat. Nothing is scripted, and it grows as his days go on.</p>
            ...
            {memList.length === 0 ? (
              <div className="mem-none"><span className="mem-pulse" /> Cybrik has not written anything down yet — his first memories will appear here as he lives. Check back in a bit.</div>
            ) : (
            ...
            <div className="doc-inner">
              <aside className="doc-nav">
                <div className="doc-nav-inner">
                  ...
                  <div className="doc-nav-foot">Cybrik World<br />Technical Reference · v0.9</div>
                </div>
              </aside>
              ...
              <div className="doc-body">
                <div className="doc-hero">
                  ...
                  <h1>Cybrik World</h1>
                  <p>A technical reference for the first memecoin community you can actually watch. It covers what Cybrik is, how the world is rendered, how he thinks and remembers, how the token breathes, and how the community drives every part of it. If you want the short version, read the Introduction and the Token section. If you want all of it, keep scrolling.</p>
                </div>

                <section id="doc-intro" className="doc-sec">
                  <div className="doc-eyebrow">01 — INTRODUCTION</div>
                  <h2>What Cybrik is</h2>
                  <p>$CYBRIK is a token whose entire community is rendered as a place. Instead of living on a chart and a chat that scrolls, the community lives in a small brick city, and at the center of it is Cybrik: a little black minifig, driven by an AI, who plays out a full crypto life on a stream that never stops.</p>
                  <p>Cybrik is one character and two things at once. He is the figure you watch on screen, and he is a stand-in for everyone in the trenches. When he apes, wins, gets rugged, gets fed or ends up in the hospital, he is doing it on behalf of the people who hold the token. The stream is not a mascot on a loop. It is the community itself, given a body and a hometown.</p>
                  <p>The reason to build it this way is simple. A memecoin is usually an abstraction: a number, a supply figure, a group chat. Cybrik turns that abstraction into somewhere you can walk into. Belonging stops being a line on a chart and becomes a place with a life going on inside it, one you can tune into at any hour and recognise as yours.</p>
                ...
                <section id="doc-world" className="doc-sec">
                  ...
                  <h3>The camera</h3>
                  <p>You are never locked to one angle. You can orbit, zoom and pan freely, snap between framed camera views, and read your coordinates and a live compass straight off the viewport. Arrow keys pan, the scroll wheel zooms, and dragging orbits. The idea is that watching Cybrik should feel like operating a camera on his world, not staring at a fixed render.</p>
                  <h3>Cybrik himself</h3>
                ...
                <section id="doc-stream" className="doc-sec">
                  ...
                  <p>The world runs on an always-on stream. Cybrik does not pause when you close the tab and pick up where you left off when you return. Whenever you open the site you are tuning into a life already in progress, the same way you would drop into any livestream partway through.</p>
                ...
                <section id="doc-brain" className="doc-sec">
                  <div className="doc-eyebrow">05 — CYBRIK’S BRAIN</div>
                  <h2>What drives him</h2>
                  <p>Cybrik is driven by Claude Fable 5, the strongest model available right now, and it is live. The model decides what he does with his day, what he says about it, and how he reacts to the market. Nothing on the feed is hand-written. It is him.</p>
                  <h3>The loop</h3>
                  ...
                  <p>Cybrik keeps a genuine 24-hour routine tied to real time: he sleeps through the night, eases in with coffee in the morning, is busiest through the day, and winds down in the evening. He never claims it is a different time of day than it actually is.</p>
                  ...
                  <p>Cybrik is a trader at heart, but no wallet or funds are connected to him yet, so he does not trade — no positions, no profit or loss. For now he watches the market, studies crypto and blockchain seriously, and lives his day, itching to be funded. The moment a wallet is connected, real trading turns on.</p>
                  <h3>His voice</h3>
                ...
                <section id="doc-memory" className="doc-sec">
                  ...
                  <p>Cybrik keeps a memory of the life he lives so he is not a fresh stranger every time you tune in. As he goes about his day he writes his own memories — a lesson, a read on the market, something he learned studying, a moment with the chat — each one typed and time-stamped.</p>
                ...
                <section id="doc-token" className="doc-sec">
                  ...
                  <p>$CYBRIK is the community, and the economy is built so that Cybrik simply living his life pulls supply out of circulation. He lives like a real person, and living costs money: food, coffee, supplies for the house, and the odd hospital bill after a rough trade.</p>
                  <div className="doc-spec">
                    <div><span>Income</span><b>Creator rewards + community</b></div>
                    <div><span>Spending</span><b>Buy back &amp; burn $CYBRIK</b></div>
                  ...
                  <h3>Where it goes</h3>
                  <p>Every time he spends, that spending is used to buy back $CYBRIK on the open market and burn it. The more life he lives, the more supply leaves for good. On top of that, when community artworks are auctioned as NFTs, the proceeds are burned too. The token does not inflate to pay for the world. The world burns the token by being lived in.</p>
                  <p>As a worked example: he has a rough day, takes a loss, and ends up covering a hospital bill and restocking the fridge. The equivalent value in $CYBRIK is bought off the market and burned. Multiply that across every real cost of a life lived on stream and the burn becomes steady and transparent, tied to genuine activity rather than to an emissions schedule.</p>
                </section>

                <section id="doc-city" className="doc-sec">
                  <div className="doc-eyebrow">08 — THE CITY</div>
                  <h2>One house, becoming a city</h2>
                  <p>Today the map is Cybrik’s house and the block around it. The long-term shape is a full city, where every building has a job and most of them are funded and voted for by holders. These are the first ones planned, and they are only the beginning.</p>
                  <ul className="doc-list">
                    ...
                    <li><b>The Cinema.</b> Where everyone gathers to watch things together, with Cybrik reacting from the front row. <em>Planned.</em></li>
                ...
                <section id="doc-community" className="doc-sec">
                  ...
                  <ul className="doc-list">
                    ...
                    <li><b>Building the city.</b> Holders use $CYBRIK to raise new buildings, expand the map, bid on the ones that already exist, and put their name on the landmarks. <em>Planned.</em></li>
                ...
                <section id="doc-subworlds" className="doc-sec">
                  ...
                  <p>The furthest-out direction is to let holders stake $CYBRIK to spin up their own worlds. Think of it as an on-chain Roblox, with Cybrik as player zero: the first inhabitant of the first world, and the proof that the format works before anyone else builds on it.</p>
                  <p>Each sub-world would be its own place with its own community, all sharing the same underlying system for building, memory and economy that Cybrik’s world runs on. Instead of one fixed scene, the world becomes a template that anyone with a stake can extend.</p>
                ...
                <section id="doc-faq" className="doc-sec">
                  ...
                  <div className="doc-qa"><b>Is Cybrik really an AI?</b><p>Yes, and he is live right now. He is driven by Claude Fable 5 — everything he does, says and remembers on the stream comes from the model in real time, grounded in the real clock and the real market.</p></div>
                  ...
                  <div className="doc-qa"><b>What makes the token supply go down?</b><p>Cybrik’s spending. Creator rewards buy back and burn $CYBRIK every time he covers a living cost, and NFT auction proceeds are burned on top of that. The burn is tied to real activity, not to an emissions schedule.</p></div>
        ...
        <section className="page-join">
          <div className="join-scroll">
            <div className="join-inner">
              ...
              <h1 className="join-title">Stake $CYBRIK.<br />Build your own world.</h1>
              <p className="join-lead">This is where the world opens up to everyone. Down the line, holders will be able to stake $CYBRIK to spin up their own sub-worlds: their own place, their own community, running on the same engine that Cybrik's world runs on. Think of it as an on-chain Roblox, with Cybrik as player zero.</p>
              ...
              <div className="join-steps">
                <div className="join-step"><span className="join-step-n">01</span><b>Stake $CYBRIK</b><p>Lock a stake to claim the right to open a sub-world of your own.</p></div>
                ...
                <div className="join-step"><span className="join-step-n">03</span><b>Bring your people</b><p>Invite your community and grow it the way Cybrik grew his own.</p></div>
              </div>
          ...
          <div className="tok-scroll">
            <div className="tok-inner">
              <div className="tok-head">
                <div className="ov-kicker">$CYBRIK</div>
                <h1 className="tok-title">One token. The whole community.</h1>
                <p className="tok-lead">$CYBRIK is not a coin sitting next to the project. It is the project. The community, the city and Cybrik himself all run on it, and the economy is built so that him simply living his life pulls supply out of circulation.</p>
              </div>
              ...
              <div className="tok-status"><span className="tok-dot" />DESIGN PHASE · CONTRACT NOT ISSUED</div>

              <div className="tok-contract">
                <span className="tok-ca-label">CONTRACT</span>
                <code style={{ wordBreak: 'break-all' }}>TO BE ANNOUNCED</code>
                <button className="tok-ca-copy" disabled>PENDING</button>
              </div>
              ...
              <div className="tok-block">
                ...
                <div className="tok-loop">
                  <div className="tok-loop-step"><span className="tok-loop-n">01</span><b>Income comes in</b><p>Creator rewards, plus whatever the community sends, keep Cybrik fed and trading.</p></div>
                  ...
                  <div className="tok-loop-step"><span className="tok-loop-n">03</span><b>Spending buys back</b><p>Every cost he covers is used to buy $CYBRIK back off the open market.</p></div>
                  <div className="tok-loop-step"><span className="tok-loop-n">04</span><b>And it burns</b><p>That $CYBRIK is burned for good. The more he lives, the more supply leaves forever.</p></div>
                </div>
              ...
              <div className="tok-block">
                ...
                <p className="tok-blurb">Every buyback and burn tied to Cybrik's spending will show up here, receipt by receipt.</p>
                <div className="tok-burn-empty"><span className="tok-dot" />No burns yet — they begin once Cybrik is funded and starts spending on-chain.</div>
              </div>

              <div className="tok-block">
                <div className="ov-label">LAUNCH STATUS</div>
                <p className="tok-blurb">The economic system is being designed alongside the world. No contract address or trading link is official until it appears here.</p>
                <div className="tok-burn-empty"><span className="tok-dot" />CYBRIK is a world first. The token comes after the system is ready.</div>
              </div>
            </div>
          </div>
        </section>
      ...
      <footer>
        <span>{datum} · {etTime}</span><span>© 2026 CYBRIK WORLD</span>
      </footer>
    </main>
  )
}
